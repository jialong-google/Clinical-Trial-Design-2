...
class getboundaryViewController: UIViewController {
    ...
    func getBoundary(target : Double, ncohort : Int, cohortsize : Int, n_earlystop: Int = 100, p_saf : Double? = nil, p_tox : Double? = nil, cutoff_eli : Double = 0.95, extrasafe : Bool = false, offset : Double = 0.05, _print : Bool = true) -> String?
    {
        var psaf : Double = 0.0
        var ptox : Double = 0.0
        ...
        if (_print) {
            res += "Escalate dose if the observed toxicity rate at the current dose <= \(lambda1)\n"
            res += "Deescalate dose if the observed toxicity rate at the current dose >= \(lambda2)\n\n"
            res += "This is equivalent to the following decision boundaries\n"
            res += String(describing: getCol(lists: boundaries, from: 1, to: (min(npts, n_earlystop)/cohortsize), step: cohortsize))
            if (cohortsize > 1) {
                res += "\n"
                res += "A more completed version of the decision boundaries is given by\n"
                
                res += String(describing: row_boundaries)
                res += String(describing : boundaries)
            }
            res += "\n"
            if (!extrasafe){
                res += "Default stopping rule: stop the trial if the lowest dose is eliminated.\n"
            }
        }
        if (extrasafe) {
            var stopbd = [Int?]()
            ...
            if (_print) {
                res += "\n"
                res += "In addition to the default stopping rule (i.e., stop the trial if the lowest dose is eliminated), \n"
                res += "the following more strict stopping safety rule will be used for extra safety: \n"
                res += " stop the trial if (1) the number of patients treated at the lowest dose >= 3 AND\n(2) Pr(the toxicity rate of the lowest dose >\(target)| data) > \(cutoff_eli - offset),\n which corresponds to the following stopping boundaries:\n"
                //diff
                res += String(describing: row_stopboundary)
                res += String(describing: col_stopboundary)
   ...
   // @IBAction func passToResults(_ sender: Any) {
        //self.showResultTextField.text = "hello world!"
        /*if self.showResultTextField != nil{
            self.showResultTextField.text = "hello world!"
        }*/
    //}
